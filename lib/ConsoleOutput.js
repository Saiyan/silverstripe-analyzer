...
var ConsoleOutputTable = function(cols) {
    var me = {};
    var columns = cols ? cols : "%s%v%da%el%et";

    var table = new clitable({
        head: getHeadFromColumns(),
        style : {compact : true}
    });
    ...
    me.addPage = function(sspage){
        table.push(
            getRowFromColumns(sspage)
        );
    };

    function getHeadFromColumns(){
        return getRowFromColumns();
    }

    function getRowFromColumns(sspage){
        var matches = columns.match(/%s|%v|%da|%el|%et|%m+/g);
        var row = [];
